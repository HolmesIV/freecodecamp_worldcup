...
echo $($PSQL "SELECT DISTINCT name FROM (SELECT * FROM games LEFT JOIN teams ON games.winner_id = teams.team_id UNION ALL (SELECT * FROM games LEFT JOIN teams ON games.opponent_id = teams.team_id)) ans WHERE round = 'Eighth-Final' AND year = 2014 ORDER BY name;")
